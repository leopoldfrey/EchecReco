...
class Makeymakey:
    ...
    def __init__(self, osc_server_port=14030, osc_client_host='0.0.0.0', osc_client_port=14028):
        ...
        self.oscClient.send("/makeymakey2/ip", self.myIp)
                        ...
                        if ev.type == ecodes.EV_KEY:
                            ...
                            if int(ee[0]) == 272 :
                                if self.verbose :
                                    print(e[3], "BTN_LEFT")
                                if e[3] == "down" : 
                                    self.oscClient.send("/makeymakey2/mouseDown", 0)
                                elif e[3] == "up" :
                                    self.oscClient.send("/makeymakey2/mouseUp", 0)
                            elif int(ee[0]) == 273 :
                                if self.verbose :
                                    print(e[2], "BTN_RIGHT")
                                if e[2] == "down" : 
                                    self.oscClient.send("/makeymakey2/mouseDown", 1)
                                elif e[2] == "up" :
                                    self.oscClient.send("/makeymakey2/mouseUp", 1)
                            elif int(ee[0]) == 274 :
                                if self.verbose :
                                    print(e[2], "BTN_MIDDLE")
                                if e[2] == "down" : 
                                    self.oscClient.send("/makeymakey2/mouseDown", 2)
                                elif e[2] == "up" :
                                    self.oscClient.send("/makeymakey2/mouseUp", 2)
                            else:
                                cod = ecodes.KEY[int(ee[0])]
                                if self.verbose :
                                    print(e[2], ee[0], cod)
                                if e[2] == "down" : 
                                    self.oscClient.send("/makeymakey2/press", (str(ee[0]) + " " +str(cod)))
                                elif e[2] == "up" :
                                    self.oscClient.send("/makeymakey2/release", (str(ee[0]) + " " +str(cod)))
                                elif e[2] == "hold" :
                                    self.oscClient.send("/makeymakey2/hold", (str(ee[0]) + " " +str(cod)))
                        elif ev.type == ecodes.EV_REL:
                            e = str(categorize(ev)).split(', ')
                            if self.verbose :
                                    print(e[1], ev.value)  
                            if e[1] == "REL_X" :
                                self.oscClient.send("/makeymakey2/relX", ev.value)
                            elif e[1] == "REL_Y" :
                                self.oscClient.send("/makeymakey2/relY", ev.value)
                            elif e[1] == "REL_WHEEL" :
                                self.oscClient.send("/makeymakey2/wheel", ev.value)
            except OSError:
    ...
    def callback(self, address, *args):
        if address == "/makeymakey2/feed" or address == "/*/feed" :
            print("Client IP : "+args[0])
            self.osc_client_host = args[0]
            self.oscClient = Client(self.osc_client_host, self.osc_client_port)
            self.oscClient.send("/makeymakey2/ready", 1)
        elif address == "/makeymakey2/test" or address == "/*/test" :
            self.oscClient.send("/makeymakey2/ready", 1)
        elif address == "/makeymakey2/verbose" or address == "/*/verbose" :
            if int(args[0]) > 0 :
                self.verbose = True
            else:
                self.verbose = False
        elif address == "/makeymakey2/devices" :
            self.devices = [InputDevice(path) for path in evdev.list_devices()]
            for device in self.devices:
                self.oscClient.send("/makeymakey2/device", str(device.path) + " " + str(device.phys) + " " + str(device.name))
        else:
